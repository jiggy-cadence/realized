...
const FORMATION = 15;
const HOLDOUT = 15;
const STEP = 7;   // v6: was 15. Tighter stepping = more windows. Holdouts now overlap; reported as a caveat, not hidden.
...
async function main() {
  // v6: the n=9 problem was never the scoring function -- it was one chain. We already
  // index 5 venues; this points the same instrument at all of them. ~5x the windows,
  // plus it lets us ask whether the effect differs BY CHAIN, which is its own question.
  const venuePools = [];
  for (const [dex, chains] of Object.entries(VENUES)) {
    for (const [chain, sub] of Object.entries(chains)) {
      try {
        const got = await fetchTopPools(gatewayUrl(KEY, sub), { first: 150, days: HISTORY_DAYS });
        console.log(`  ${dex}/${chain}: ${got.length} pools`);
        for (const g of got) venuePools.push({ ...g, __venue: `${dex}/${chain}` });
      } catch (e) { console.log(`  ${dex}/${chain}: FAILED ${String(e.message).slice(0,60)}`); }
    }
  }
  const pools = venuePools;
  console.log(`fetched ${pools.length} pools across ${new Set(pools.map(x=>x.__venue)).size} venues\n`);
  ...
  for (const p of pools) {
    ...
    if (days.length >= HISTORY_DAYS * 0.7) series.push({ id: p.id, venue: p.__venue, days });
  }
  ...
  console.log('pools with enough history, by venue:');
  for (const [v, arr] of byVenue) console.log(`  ${v}: ${arr.length}`);
  console.log('');

  const windows = [];
  for (const [venueName, venueSeries] of byVenue) {
    if (venueSeries.length < 20) { console.log(`  skip ${venueName}: only ${venueSeries.length} pools`); continue; }
    const minLen = Math.min(...venueSeries.map((s) => s.days.length));
    for (let start = 0; start + FORMATION + HOLDOUT <= minLen; start += STEP) {
    const scored = [];
    for (const s of venueSeries) {
    ...
    const picked = bySc.slice(0, k);
    // v8: also keep the BOTTOM decile. v7 saved only the top, so "does betting
    // against the score work on Arbitrum" was un-testable from stored output --
    // the data to answer it was computed and then thrown away every window.
    const bottom = bySc.slice(-k);
    ...
    windows.push({
      ...
      // v7: keep the actual picks. v6 threw these away, so a -4.2pp window was
      // un-diagnosable -- I could see the aggregate died but not WHICH pool killed it.
      bottomMeanPct: Number((mean(bottom.map((x) => x.holdout))).toFixed(3)),
      bottomDetail: bottom.map((x) => ({
        id: x.id,
        holdoutPct: Number(x.holdout.toFixed(3)),
        score: Number(x.score.toFixed(3)),
      })).sort((a, b) => a.holdoutPct - b.holdoutPct),
      pickDetail: picked.map((x) => ({
        id: x.id,
        holdoutPct: Number(x.holdout.toFixed(3)),
        score: Number(x.score.toFixed(3)),
        drift: x.formationDrift === null ? null : Number(x.formationDrift.toFixed(4)),
      })).sort((a, b) => a.holdoutPct - b.holdoutPct),
      // trimmed mean: drop best+worst pick. If the edge survives trimming, the
      // strategy is broad; if it collapses, it was one pool carrying it.
      pickedTrimmedMeanPct: (() => {
        const v = picked.map((x) => x.holdout).sort((a, b) => a - b);
        if (v.length < 3) return null;
        const t = v.slice(1, -1);
        return Number((t.reduce((a, b) => a + b, 0) / t.length).toFixed(3));
      })(),
  ...
  console.log('WALK-FORWARD v8 -- both tails captured (inversion testable)');
  ...
  // v6: per-venue breakdown -- Jiggy's question, "is there a pattern BETWEEN chains".
  const venues = [...new Set(windows.map((w) => w.venue))];
  console.log('  by venue:');
  for (const v of venues) {
    const ws = windows.filter((w) => w.venue === v);
    const es = ws.map((w) => w.edgePp);
    const pos = es.filter((e) => e > 0).length;
    console.log(`    ${v.padEnd(22)} n=${String(ws.length).padStart(2)}  median ${String(median(es).toFixed(3)).padStart(7)}pp  mean ${String(mean(es).toFixed(3)).padStart(7)}pp  positive ${pos}/${ws.length}  meanP ${mean(ws.map((w) => w.randomP)).toFixed(3)}`);
  }
  console.log('');
  console.log('  per-window:');
  for (const w of windows) {
    console.log(`    ${w.venue.padEnd(20)} d${String(w.startDayIndex).padStart(3)}  picked-mean ${String(w.pickedMeanPct).padStart(8)}%  all-median ${String(w.allMedianPct).padStart(8)}%  edge ${String(w.edgePp).padStart(8)}pp  p=${String(w.randomP).padEnd(6)}  drift pick ${String(w.pickedMedianDrift).padStart(6)} vs all ${String(w.allMedianDrift).padStart(6)}`);
  }

  writeFileSync(new URL('../data/spike-hunt8.json', import.meta.url), JSON.stringify({
